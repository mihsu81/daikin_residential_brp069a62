...
HA_DHW_MODE_TO_DAIKIN = {
    ...
    STATE_HEAT_PUMP: ATTR_DHW_STATE_HEAT_PUMP,
    STATE_OFF: ATTR_DHW_STATE_OFF,
}

HA_DHW_ATTR_TO_DAIKIN = {
    ATTR_DHW_MODE: ATTR_DHW_MODE_SET,
    ATTR_DHW_TARGET_TEMPERATURE: ATTR_DHW_TARGET_TEMPERATURE,
}
...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    for dev_id, device in hass.data[DAIKIN_DOMAIN][DAIKIN_DEVICES].items():
        _LOGGER.debug("Daikin DHW Tank Add: {}".format(device))
    ...
    async def _set(self, settings):
        ...
        for attr in [ATTR_TEMPERATURE, ATTR_DHW_MODE]:
            ...
            # temperature
                ...
                try:
                    values[HA_DHW_ATTR_TO_DAIKIN[ATTR_DHW_TARGET_TEMPERATURE]] = str(int(value))
                except ValueError:
